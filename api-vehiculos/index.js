...
var db = mongojs("mongodb+srv://fran:fqOZ39V1T0kItdyz@cluster0.biqs9.mongodb.net/vehiculos?retryWrites=true&w=majority"); // Enlazamos con la DB "Atlas"
...
app.delete('/api/reserva/:id',auth, (req, res, next) => {//cancela reservas
    ...
    db.collection("ofertas").findOne(id(elementoId),(err,elemento)=>{//busco
        ...
        if(elemento.disponible=="no"){//miro si esta reservado
            ...
            res.json({
                vuelo:elemento,
                mensaje:"El vehiculo no estaba reservado previamente"
            })
...
https.createServer(opciones,app).listen(port,()=>{
    console.log(`API vehiculos ejecutándose en https://localhost:${port}/api/ofertas/id`);
}); 
